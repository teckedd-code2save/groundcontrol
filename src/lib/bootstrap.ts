...
apt-get update && apt-get install -y caddy 2>&1`;
  ...
  const script = `curl -fsSL -o /usr/local/bin/caddy "https://github.com/caddyserver/caddy/releases/latest/download/caddy_linux_${goArch}" && \
...
export async function installNginx(vps?: VpsConnection | null): Promise<BootstrapResult> {
  ...
  const arch = await execOnTarget("uname -m", conn);
  const goArch = arch.stdout.trim() === "aarch64" ? "arm64" : arch.stdout.trim() === "x86_64" ? "amd64" : arch.stdout.trim();
  const script = `curl -fsSL -o /tmp/nginx.tar.gz "https://nginx.org/download/nginx-1.26.2-linux-${goArch}.tar.gz" && \
...
export async function installCloudflared(vps?: VpsConnection | null): Promise<BootstrapResult> {
  ...
  const script = `curl -fsSL -o /usr/local/bin/cloudflared "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-${goArch}" && chmod +x /usr/local/bin/cloudflared && /usr/local/bin/cloudflared version 2>&1`;
  const result = await execOnTarget(script, conn);
  return { success: result.code === 0, output: result.stdout, error: result.stderr };
}
...
curl -fsSL -o /tmp/terraform.zip "https://releases.hashicorp.com/terraform/\${VERSION}/terraform_\${VERSION}_linux_${tfArch}.zip" && \
...
curl -fsSL -o /usr/local/bin/kubectl "https://dl.k8s.io/release/$KUBECTL_VERSION/bin/linux/${kubeArch}/kubectl" && \

...
/** compose = docker compose stack; static = git/local files served by reverse proxy (no Docker). */
export type TemplateDeployMode = "compose" | "static";
...
function parseTemplateYaml(content: string): TemplateDefinition {
  ...
  const deployModeRaw = String(doc.deploy_mode || doc.deployMode || "").toLowerCase();
  const deployMode: TemplateDeployMode =
    deployModeRaw === "static" || category === "static"
      ? "static"
      : "compose";

  // Static templates never require Docker. Compose templates default to docker:true
  // unless explicitly disabled in YAML.
  const requiresDocker =
    deployMode === "static"
      ? false
      : requires.docker !== false;
...
export function resolveTemplate(
  ...
  // Template-declared inputs first; then pass through extra deploy-time keys
  // (e.g. static_dir, repo_url) so proxy/compose templates can resolve them.
  const resolvedInputs = {
    ...userInputs,
    ...resolveInputs(template.inputs, userInputs),
  };
...
function generateDockerCompose(
  ...
  if (isStaticTemplate(template) || template.services.length === 0) {
    return [
      ...
      `# Generated by GroundControl — ${template.name} v${template.version}`,
      `# ${new Date().toISOString()}`,
      `# Files are published under the reverse proxy root (see proxy config).`,
      `# Source: ${resolved.repo_url || userInputs.repo_url || resolved.repo_dir || userInputs.repo_dir || "(set at deploy)"}`,
      `# Publish dir: ${resolved.output_dir || "."}`,
    ].join("\n");
  }
    ...
    if (template.reverse_proxy.sites && template.reverse_proxy.sites.length > 0) {
      for (const site of template.reverse_proxy.sites) {
        ...
        if (site.root) {
          ...
          // SPA-friendly: fall back to index.html for client-routed paths
...
http:
  ...
  // nginx
  if (template.reverse_proxy.sites?.some((site) => site.root)) {
    const lines = [`# Nginx static config for ${domain}`, `server {`, `  listen 80;`, `  listen 443 ssl;`, `  server_name ${domain};`, ``];
    for (const site of template.reverse_proxy.sites || []) {
      if (!site.root) continue;
      const d = resolveTemplateString(site.domain, resolved);
      const root = resolveTemplateString(site.root, resolved);
      lines.push(`  # site: ${d}`);
...
export function generatePreview(resolved: ResolvedTemplate): string {
  ...
  if (isStaticTemplate(t)) {
    lines.push("## Static site");
    lines.push(`  • No Docker / Compose — files served directly by ${t.reverse_proxy.type}`);
    lines.push(`  • Publish directory: ${resolved.inputs.output_dir || "."}`);
    if (resolved.inputs.build_command) {
      lines.push(`  • Build command: ${resolved.inputs.build_command}`);
    } else {
      lines.push(`  • Build command: (none — plain HTML/CSS/JS)`);
    }
  ...
  lines.push(`  • .groundcontrol/manifest.json`);
  lines.push("");

  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      ctx = gsap.context(() => {
        ...
        gsap.fromTo(".hero-sub", { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, delay: 0.6, ease: "power2.out" });
        gsap.fromTo(".hero-cta", { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, delay: 0.9, ease: "power2.out" });
        // SVG line draw
        ...
        gsap.fromTo(".stat-number", { innerText: 0 },
          { innerText: (i: number, el: any) => el.getAttribute("data-value"), duration: 2, snap: { innerText: 1 },
        ...
        gsap.to(".orb-3", { y: -60, x: -40, scrollTrigger: { trigger: "body", start: "top top", end: "bottom bottom", scrub: 1.5 } });
      });
    }
    init();
    return () => ctx?.revert();
  ...
  return (
    <div className="bg-black text-white min-h-screen overflow-x-hidden" style={{ fontFamily: "'Schibsted Grotesk', system-ui, sans-serif" }}>
      ...
      <style>{`
        ...
        .glow { box-shadow: 0 0 80px rgba(232,84,42,0.15), 0 0 200px rgba(232,84,42,0.05); }
      `}</style>
      ...
      <div className="orb-3 fixed top-1/2 left-1/3 w-[400px] h-[400px] rounded-full opacity-10 pointer-events-none z-0" style={{ background: "radial-gradient(circle, rgba(255,106,64,0.2) 0%, transparent 70%)" }} />
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <svg className="hero-svg absolute inset-0 w-full h-full z-0 opacity-[0.06]" viewBox="0 0 1440 900" preserveAspectRatio="none">
          ...
          {[200,400,600,800,1000,1200].map((x,i) => <line key={`v${x}`} x1={x} y1="100" x2={x} y2="800" stroke="rgba(232,84,42,0.5)" strokeWidth="0.5" />)}
          ...
          <circle cx="300" cy="300" r="4" fill="#E8542A" />
          <circle cx="600" cy="200" r="4" fill="#E8542A" />
          <circle cx="900" cy="350" r="4" fill="#E8542A" />
          <circle cx="700" cy="600" r="4" fill="#FF6A40" />
        </svg>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center py-20">
          ...
          <div className="hero-cta flex flex-wrap items-center justify-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <code className="hidden sm:block px-4 py-3 bg-white/[0.04] border border-white/[0.08] rounded-xl text-xs font-mono text-white/25">
                {installCmd}
              </code>
              <button onClick={copyCmd}
                className="px-3 py-3 text-xs font-mono text-white/20 hover:text-[#E8542A] border border-white/[0.08] rounded-xl hover:border-[#E8542A]/30 transition-all shrink-0">
                {copied ? "Copied" : "Copy"}
              </button>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-20">
            ...
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">
              ...
              <span className="gradient-text">run production</span>
            </h2>
      ...
      {/* ══════════════════ STATS ══════════════════ */}
      <section className="stats-section relative py-32 border-t border-white/[0.06]">
        <div className="max-w-4xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              { value: "476", label: "GitHub Stars", suffix: "K+" },
              { value: "22", label: "AI Agent Tools" },
              { value: "5", label: "Cloud Platforms" },
              { value: "3", label: "Deploy Templates" },
            ].map((s, i) => (
              <div key={i}>
      ...
      <section className="relative py-32 border-t border-white/[0.06]">
        ...
        <div className="relative max-w-2xl mx-auto px-6 text-center">
          <div className="bg-white/[0.02] border border-white/[0.06] rounded-3xl p-12 glow">
            ...
            <div className="flex items-center gap-2 justify-center mb-6">
              ...
              <button onClick={copyCmd}
                className="px-4 py-3 text-xs font-mono text-white/20 hover:text-[#E8542A] border border-white/[0.08] rounded-xl hover:border-[#E8542A]/30 transition-all shrink-0">
                {copied ? "✓" : "Copy"}
              </button>
            </div>
            <div className="flex gap-4 justify-center">
              <button onClick={() => setShowLogin(true)}
                className="px-6 py-3 bg-[#E8542A] text-white rounded-xl hover:bg-[#FF6A40] transition-all text-sm font-mono shadow-lg shadow-[#E8542A]/20">
                Sign In
              </button>
              <Link href="/"
                className="px-6 py-3 border border-white/[0.08] rounded-xl hover:border-[#E8542A]/30 hover:text-[#E8542A] transition-all text-sm font-mono">
                Learn More →
              </Link>
            </div>
          </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-5">
                <a href="https://github.com/teckedd-code2save/groundcontrol" target="_blank" rel="noopener"
                  className="text-xs font-mono text-white/25 hover:text-[#E8542A] transition-colors">GitHub</a>
                <a href="https://github.com/teckedd-code2save/convoy" target="_blank" rel="noopener"
                  className="text-xs font-mono text-white/25 hover:text-[#E8542A] transition-colors">Convoy</a>
                <a href="https://www.serendepify.com" target="_blank" rel="noopener"
                  className="text-xs font-mono text-white/25 hover:text-[#E8542A] transition-colors">Serendepify</a>
              </div>
            </div>
      ...
      {showLogin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-black border border-white/[0.08] rounded-2xl p-8 w-full max-w-sm shadow-2xl glow">
            ...
            <p className="text-xs text-white/30 mb-6">Access your GroundControl dashboard</p>
            ...
            <button onClick={() => setShowLogin(false)}
              className="mt-4 w-full text-center text-xs text-white/15 hover:text-white/30 transition-colors font-mono">
              Cancel
            </button>
          </div>
        </div>

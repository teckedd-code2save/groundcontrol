...
The production Intelligence workspace can read the active host, reconcile Docker/Compose and Caddy/Nginx evidence, record meaningful changes, run an external HTTP check for every observed public route, and produce evidence-backed investigations. A failed public check is a first-class operational event: GroundControl tests the configured loopback upstream from the deployment host, checks the listening-port contract, correlates Docker/Compose identity, and isolates the first broken boundary. The interface presents this as a compact operator decision: problem, proposed fix, live investigation, and verification. Detailed evidence remains available on demand; internal state-machine narration does not occupy the primary surface.

...
The demo data is **opt-in** via the `GC_SEED_DEMO` environment variable, layered on top of the normal admin seed:

```bash
# Default seed only (admin user):
npm run db:seed
...
Then run the app and log in with `admin` / `groundcontrol2024`:
...
| Model | Rows | Contents |
|-------|------|----------|
| `User` | 1 | Default `admin` (from the base seed; created even without the flag) |
